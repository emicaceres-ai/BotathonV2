...
interface ActionPanelProps {
  ...
  onOpenAddModal: () => void;
  onRPAAction?: (type: 'retencion' | 'capacitacion') => void;
  onGenerateInsight?: () => void;
  rpaLoading?: string | null;
  aiLoading?: boolean;
    ...
    <div className={`rounded-xl shadow-sm p-6 mb-8 border transition-all ${isHighContrast ? 'bg-black border-white' : 'bg-white border-gray-100'}`}>
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 flex flex-wrap gap-4">
        {onRPAAction && (
          <>
            <button
              onClick={() => onRPAAction('retencion')}
              disabled={rpaLoading !== null || filteredCount === 0}
            ...
            >
              {rpaLoading === 'retencion' ? <Loader2 className="animate-spin mr-2 h-4 w-4"/> : <Zap className="mr-2 h-4 w-4" />}
              Comunicación de Retención
            </button>

            <button
              onClick={() => onRPAAction('capacitacion')}
              disabled={rpaLoading !== null || !gapOnly}
              className={`flex items-center justify-center px-6 py-3 rounded-lg font-bold text-sm transition-all shadow-sm active:scale-95 ${
                isHighContrast 
